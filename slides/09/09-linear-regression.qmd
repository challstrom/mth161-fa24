...
-   $y$ is the **response** or **outcome** or
    **dependent** variable. In prediction problems, this is what we are
    interested in predicting.
...
$$ \hat{y} = b_0 + b_1 \cdot \hat{x} $$ 
Next we use the fact that $(\bar{x}, \bar{y})$ is on the line.  Plug these values into line equation

$$\bar{y} = b_0 + b_1 \cdot \bar{x} $$
Now everything is known except $b_0$ so we can solve for that!
