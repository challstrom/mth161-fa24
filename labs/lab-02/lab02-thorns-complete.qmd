...
Thorns win more than they lose or tie
:::
...
# scale_fill_manual(values = c("black", "#EE202F", "#99242b"))
```
...
thorns |>
  ...
  mutate( pct = n / sum(n)) |>
  pivot_wider(id_cols = home_thorns, names_from = result , values_from = pct )
```

::: {.callout-note title="Task 3 Response" icon="false"}
Based on your findings, do you think that `results` and `home_thorns` are associatated? Does this suggest the Thorns have a home-field advantage? Why or why not?

*Thorns win more at home than they do when they're away. These variables do seem to have an association.*
:::
...
*thorns score an average of about 2 goals per game at home. in away games, they score an average of 1.25 goals.*
:::
...
Examine this table and make some observations.
:::
...
chisq_test( thorns, home_thorns ~ result)
```
